...
export default function Cards() {
  ...
  const handleSaveResponsavelRapida = async (notaId, userId) => {
    ...
    const { data: notaAtual, error: fetchError } = await supabase
      ...
      .eq("id", notaId)
      .single();
    if (fetchError) { alert("Erro ao atualizar responsável."); return; }
    ...
    else alert("Erro ao salvar responsável.");
  };
  ...
  const handleRemoveResponsavelRapida = async (notaId, userIdToRemove = null) => {
    ...
    const novosIds = userIdToRemove
      ? (notaAtual.responsaveis_ids || []).filter(id => id !== userIdToRemove)
      : [];
    const { error } = await supabase
      .from("notas")
      .update({ responsaveis_ids: novosIds })
  ...
  const handleCreateColumn = async (tipo) => {
    ...
    if (newPilha) {
      ...
      if (modoArquivadas) setColumnsArquivadas(prev => [...prev, novaCol]);
      else setColumnsNormais(prev => [...prev, novaCol]);
    }
  };
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      const {
        projectId, setorId, projectName, setorName, projectPhoto, setorPhoto,
        entityType: typeFromState, containerId: containerIdFromState,
      } = location.state || {};
      ...
      try {
        ...
        if (type === "project") {
          const { data: projData, error: projError } = await supabase
            .from("projects").select("*").eq("id", entityId).single();
          if (projError) throw new Error("Projeto não encontrado");
          entityData = projData;
        } else {
          const { data: setData, error: setError } = await supabase
            .from("setores").select("*").eq("id", entityId).single();
        ...
        const { data: membersData } = await supabase
          .from(memberTable).select("user_id").eq(idField, entityId);
        ...
        const { data: pilhasNormaisNull } = await supabase
          .from("pilhas").select("*").eq(idField, entityId)
          .is("arquivada", null).order("ordem", { ascending: true });
        const { data: pilhasNormaisFalse } = await supabase
          .from("pilhas").select("*").eq(idField, entityId)
          .eq("arquivada", false).order("ordem", { ascending: true });
        const { data: pilhasArquivadasRaw } = await supabase
          .from("pilhas").select("*").eq(idField, entityId)
          .eq("arquivada", true).order("ordem", { ascending: true });
          ...
          Promise.all(
            pilhas.map(async (pilha) => {
              const { data: notasRaw } = await supabase
                .from("notas").select("*").eq("pilha_id", pilha.id)
        ...
        const initEstado = (pilhas) => {
          const progresso = {}, concluidas = new Set(),
                dataConclusao = {}, dataEntrega = {};
          pilhas.forEach(p =>
            p.notas.forEach(n => {
        ...
        const { progresso: progN, concluidas: concN, dataConclusao: dataN, dataEntrega: dataE } =
          initEstado(pilhasNormaisComNotas);
        ...
        if (notaIdFromUrl) {
          ...
          if (notaArquivada) {
            ...
            if (col?.title === "Recebidos") { setIsNotaRecebidos(true); loadOrigemData(notaArquivada.id); }
          } else if (notaNormal) {
            setNotaSelecionada(notaNormal);
            const col = pilhasNormaisComNotas.find(c => c.notas.some(n => n.id === notaNormal.id));
            if (col?.title === "Recebidos") { setIsNotaRecebidos(true); loadOrigemData(notaNormal.id); }
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (notaSelecionada && notaId && String(notaSelecionada.id) === notaId) return;

    if (!notaId && notaSelecionada) {
      const timer = setTimeout(() => {
        const check = new URLSearchParams(window.location.search).get("nota");
        if (!check && notaSelecionada) {
    ...
    if (notaId && columnsAtivas.length > 0) {
      ...
      for (const col of columnsAtivas) {
        const nota = col.notas.find(
          n => String(n.id) === notaId && TIPOS_COM_VISUALIZACAO.includes(n.tipo)
        );
        if (nota) {
      ...
      if (!found && (!notaSelecionada || String(notaSelecionada.id) !== notaId)) {
        supabase
          .from("notas").select("*").eq("id", notaId).single()
          .then(({ data: nota, error }) => {
            if (!error && nota && TIPOS_COM_VISUALIZACAO.includes(nota.tipo)) {
              setNotaSelecionada(nota);
              setIsNotaRecebidos(false);
            }
          });
  ...
  const loadOrigemData = async (notaEspelhoId) => {
    try {
      const { data: notaEspelho } = await supabase
        ...
        .eq("id", notaEspelhoId).single();
      if (notaEspelho?.projeto_origem_id) {
        const { data: projeto } = await supabase
          .from("projects").select("id, name").eq("id", notaEspelho.projeto_origem_id).single();
        setProjetoOrigem(projeto || null);
      }
      if (notaEspelho?.nota_original_id) {
        const { data: nota } = await supabase
          .from("notas").select("id, nome").eq("id", notaEspelho.nota_original_id).single();
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (!cardsBody) return;
      if (e.key === "ArrowLeft")  { e.preventDefault(); cardsBody.scrollBy({ left: -300, behavior: "smooth" }); }
      if (e.key === "ArrowRight") { e.preventDefault(); cardsBody.scrollBy({ left:  300, behavior: "smooth" }); }
    };
  ...
  const toggleConclusaoNota = async (notaId, concluida) => {
    ...
    if (!error) {
      setNotasConcluidas(prev => {
        ...
        newConcluida ? novo.add(String(notaId)) : novo.delete(String(notaId));
        return novo;
      });
    }
  };
  ...
  const saveEditedNota = async () => {
    ...
    if (!error) {
      ...
      setter(prev =>
        prev.map(c =>
          c.id === pilhaId ? { ...c, notas: c.notas.map(n => n.id === id ? { ...n, nome, responsavel } : n) } : c
        )
      );
  ...
  const saveDataEntrega = async (notaId, data) => {
    ...
    if (!error) {
      ...
      update(setColumnsNormais);
      update(setColumnsArquivadas);
    }
  };
  ...
  const onDragEnd = useCallback(async (result) => {
    ...
    if (type === "CARD") {
      ...
      try {
        ...
        if (notaSelecionada?.id === movedNote.id && TIPOS_COM_VISUALIZACAO.includes(movedNote.tipo)) {
  ...
  return (
    <div className={`cards-page ${modoArquivadas ? "arquivo-cards-page" : ""}`}>
      <CardsHeader
        entity={entity}
        membros={membros}
        donoContainerId={donoContainerId}
