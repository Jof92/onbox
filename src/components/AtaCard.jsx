...
export default function AtaCard({ projetoAtual, pilhaAtual, notaAtual, ultimaAlteracao, onProgressoChange }) {
  ...
  const fetchProjeto = async () => {
    ...
    try {
      const { data } = await supabase.from("projects").select("name").eq("id", projetoAtual.id || projetoAtual).single();
  ...
  const obterLocalizacao = async () => {
    const fmt = cidade => `${cidade || ""}, ${new Date().toLocaleDateString("pt-BR", { day: "numeric", month: "long", year: "numeric" })}`;
    try {
  ...
  const fetchAta = async () => {
    ...
    try {
      const { data: ata } = await supabase.from("atas").select("*")
      ...
      if (!ata) {
        setAtaId(null);
        setPauta(""); setLocal(""); setTexto(""); setProxima(""); setCriarObjetivos(false);
  ...
  const extrairObjetivos = txt => {
    ...
    txt.split(/\n/).forEach(linha => {
      ...
      indices.forEach((start, i) => {
        ...
        if (trecho && !objetivos.some(o => o.texto === trecho)) objetivos.push({ texto: trecho, responsavelId: null, responsavelNome: "", dataEntrega: "" });
      });
    });
  ...
  const buscarSugestoes = async (query, campo) => {
    if (!query.startsWith("@") || query.length < 2) return campo === "participante" ? setSugestoesParticipantes([]) : setSugestoesResponsavel(prev => ({ ...prev, [campo]: [] }));
    try {
      const { data } = await supabase.from("profiles").select("id,nome,funcao").ilike("nome", `%${query.slice(1)}%`).limit(10);
      campo === "participante" ? setSugestoesParticipantes(data || []) : setSugestoesResponsavel(prev => ({ ...prev, [campo]: data || [] }));
    } catch {
      campo === "participante" ? setSugestoesParticipantes([]) : setSugestoesResponsavel(prev => ({ ...prev, [campo]: [] }));
    }
  };
  ...
  const handleResponsavelChange = (e, i) => { const n = [...objetivosList]; n[i].responsavelId = null; n[i].responsavelNome = e.target.value; setObjetivosList(n); buscarSugestoes(e.target.value, i); };
  const selecionarResponsavel = (item, i) => { const n = [...objetivosList]; n[i].responsavelId = item.id; n[i].responsavelNome = item.nome; setObjetivosList(n); setSugestoesResponsavel(prev => ({ ...prev, [i]: [] })); };
  ...
  const salvarAta = async () => {
    try {
      ...
      for (const p of participantes) if (!p.id.toString().startsWith("ext")) await supabase.from("ata_participantes").insert([{ ata_id: savedAta.id, profile_id: p.id }]);
      for (const [i, o] of objetivosList.entries()) await supabase.from("ata_objetivos").insert([{ ata_id: savedAta.id, texto: o.texto, responsavel_id: o.responsavelId || null, data_entrega: o.dataEntrega || null, concluido: objetivosConcluidos.includes(i) }]);
          ...
          <div key={campo} className="ata-section">
            {editing[campo] ?
              <input
                ...
                autoFocus
              /> :
              <span className={`${campo}-text`} onDoubleClick={() => setEditing({ ...editing, [campo]: true })}>
        ...
        <div className="ata-section">
          <input
            ...
            onKeyDown={e => {
              if (e.key === "Enter") {
                e.preventDefault();
                sugestoesParticipantes.length > 0 ? selecionarSugestao(sugestoesParticipantes[0]) : participanteInput.trim() && setParticipantes(prev => [...prev, { id: `ext-${Date.now()}`, nome: participanteInput.trim(), funcao: "Externo" }]) && setParticipanteInput("");
              }
            }}
          />
            ...
            <div className="ata-objectives">
              {objetivosList.map((o, i) =>
                <div key={i} className="objetivo-item">
                  ...
                  <input type="date" value={o.dataEntrega || ""} onChange={e => { const n = [...objetivosList]; n[i].dataEntrega = e.target.value; setObjetivosList(n); }} />
                </div>
              )}
            </div>
