...
const Rdo = ({ notaId, onClose, usuarioId, projetoAtual }) => {
  ...
  const [data, setData] = useState({
    ...
    recebimento_materiais: [{ codigo: "", descricao: "", unidade: "", quantidade: "", empresa: "" }],
    equipamentos: [{ codigo: "", descricao: "", total: "", em_uso: "" }],
    intercorrencias: "",
    tem_intercorrencias: false,
  ...
  // ─── FIX: ref para controlar se clicou numa sugestão (evita fechar antes do onClick) ───
  const clicandoSugestaoRef = useRef(false);
      ...
      try {
        ...
        if (rdoData) {
          ...
          setData({
            ...
            tem_intercorrencias: temIntercorrencias,
  ...
  // ─── FIX: busca por nome E nickname, sem filtro de termo vazio ─────────────
  const buscarMembrosDoProjetoRdo = async (termo) => {
    ...
    if (erroMembros || !membros?.length) return [];

    const ids = membros.map(m => m.user_id).filter(Boolean);
    if (ids.length === 0) return [];

    // ─── FIX: busca por nome, nickname e email — sem filtro se termo vazio ───
    let query = supabase
      .from("profiles")
    ...
    // Se tiver termo digitado, filtra; senão mostra todos os membros
    if (termo.trim()) {
      query = query.or(
        `nome.ilike.%${termo}%,nickname.ilike.%${termo}%`
      );
    }

    const { data: perfis, error: erroPerfis } = await query;
    if (erroPerfis) {
      console.error("Erro ao buscar membros:", erroPerfis);
      return [];
    }
  ...
  const handleResponsavelChange = async (e) => {
    ...
    if (arroba !== -1) {
      // Tudo depois do @ até o cursor é o termo de busca
      const termo = antes.substring(arroba + 1);
      setMostrarSugestoesMembros(true);
      // Busca mesmo com termo vazio (mostra todos os membros ao digitar @)
  ...
  // ─── FIX: substitui apenas o trecho @termo pelo nome escolhido ────────────
  const inserirResponsavel = (perfil) => {
    const nomeEscolhido = perfil.nickname || perfil.nome || "";
    const valorAtual = data.responsavel_preenchimento;
    const pos = responsavelInputRef.current?.selectionStart ?? valorAtual.length;

    const antes = valorAtual.substring(0, pos);
    const depois = valorAtual.substring(pos);
    const arroba = antes.lastIndexOf("@");

    // Substitui do @ até o cursor pelo nome escolhido
    const novoValor = antes.substring(0, arroba) + nomeEscolhido + depois;

    setData(prev => ({ ...prev, responsavel_preenchimento: novoValor }));
    setMostrarSugestoesMembros(false);
    setSugestoesMembros([]);
    clicandoSugestaoRef.current = false;

    setTimeout(() => {
      if (responsavelInputRef.current) {
        const novaCursor = arroba + nomeEscolhido.length;
        responsavelInputRef.current.focus();
        responsavelInputRef.current.setSelectionRange(novaCursor, novaCursor);
      }
    }, 0);
  };
  ...
  const addRow = (key) => {
    ...
    if (key === "equipamentos") {
      ...
      row = { codigo: "", descricao: "", unidade: "", quantidade: "", empresa: "" };
    } else {
  ...
  const saveRdo = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        atividades,
        intercorrencias: data.tem_intercorrencias ? data.intercorrencias : "",
        tem_intercorrencias: data.tem_intercorrencias,
  ...
  return (
    <div className="rdo-modal-container">
      ...
      <div className="rdo-content">
            ...
            <div className="rdo-col">
              ...
              <div className="relative-input-wrapper">
                <input
                  ...
                  placeholder="Digite @ para ver membros do projeto"
                  autoComplete="off"
                  // ─── FIX: só fecha se não estiver clicando numa sugestão ───
                  onBlur={() => {
                    setTimeout(() => {
                      if (!clicandoSugestaoRef.current) {
                        setMostrarSugestoesMembros(false);
                      }
                    }, 150);
                  }}
                ...
                {mostrarSugestoesMembros && sugestoesMembros.length > 0 && (
                  <div className="suggestions-dropdown">
                    {sugestoesMembros.map(membro => (
                      <div
                        key={membro.id}
                        // ─── FIX: marca que está clicando antes do onBlur disparar ───
                        onMouseDown={() => { clicandoSugestaoRef.current = true; }}
                        onClick={() => inserirResponsavel(membro)}
                        className="suggestion-item"
                      >
                        <div className="suggestion-avatar-placeholder">
                          {(membro.nickname || membro.nome)?.charAt(0).toUpperCase() || "?"}
                        </div>
          ...
          <table>
            ...
            <tbody>
              <tr>
                <td>Manhã</td>
                <td>
                  <div className={`rdo-clima-icon ${data.clima_manha === "chuvoso" ? "selected" : ""}`} onClick={() => updateField("clima_manha", "chuvoso")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("clima_manha", "chuvoso")} aria-label="Chuvoso">
                    <FontAwesomeIcon icon={faCloudShowersHeavy} />
                  </div>
                  <div className={`rdo-clima-icon ${data.clima_manha === "seco" ? "selected" : ""}`} onClick={() => updateField("clima_manha", "seco")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("clima_manha", "seco")} aria-label="Ensolarado">
                    <FontAwesomeIcon icon={faSun} />
                  </div>
                </td>
                <td>
                  <div className={`rdo-op-text ${data.obra_op_manha === "sim" ? "selected" : ""}`} onClick={() => updateField("obra_op_manha", "sim")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("obra_op_manha", "sim")} aria-label="Sim">Sim</div>
                  <div className={`rdo-op-text ${data.obra_op_manha === "nao" ? "selected" : ""}`} onClick={() => updateField("obra_op_manha", "nao")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("obra_op_manha", "nao")} aria-label="Não">Não</div>
                </td>
              </tr>
              <tr>
                <td>Tarde</td>
                <td>
                  <div className={`rdo-clima-icon ${data.clima_tarde === "chuvoso" ? "selected" : ""}`} onClick={() => updateField("clima_tarde", "chuvoso")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("clima_tarde", "chuvoso")} aria-label="Chuvoso">
                    <FontAwesomeIcon icon={faCloudShowersHeavy} />
                  </div>
                  <div className={`rdo-clima-icon ${data.clima_tarde === "seco" ? "selected" : ""}`} onClick={() => updateField("clima_tarde", "seco")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("clima_tarde", "seco")} aria-label="Ensolarado">
                    <FontAwesomeIcon icon={faSun} />
                  </div>
                </td>
                <td>
                  <div className={`rdo-op-text ${data.obra_op_tarde === "sim" ? "selected" : ""}`} onClick={() => updateField("obra_op_tarde", "sim")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("obra_op_tarde", "sim")} aria-label="Sim">Sim</div>
                  <div className={`rdo-op-text ${data.obra_op_tarde === "nao" ? "selected" : ""}`} onClick={() => updateField("obra_op_tarde", "nao")} role="button" tabIndex={0} onKeyPress={(e) => e.key === "Enter" && updateField("obra_op_tarde", "nao")} aria-label="Não">Não</div>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div className="rdo-section">
          <div className="rdo-section-header">
            ...
            <div className="rdo-add-button-group">
              <img src={dataTransferImage} alt="Copiar do último RDO" className="data-transfer-icon" onClick={() => copiarDoUltimoRdo("atividades")} width="24" height="24" />
              <button type="button" className="rdo-add-pavimento-btn" onClick={() => setMostrarPavimentosDisponiveis(prev => !prev)} disabled={pavimentosDisponiveis.length === 0} title={pavimentosDisponiveis.length === 0 ? "Todos os pavimentos já estão incluídos" : "Adicionar pavimento"}>+</button>
            </div>
          </div>
          ...
          {mostrarPavimentosDisponiveis && pavimentosDisponiveis.length > 0 && (
            <div className="rdo-pavimento-select-dropdown">
              {pavimentosDisponiveis.map((pav) => (
                <div key={pav} className="rdo-pavimento-select-item" onClick={() => { setPavimentosAtividades(prev => [...prev, { pavimento: pav, descricao: "" }]); setMostrarPavimentosDisponiveis(false); }}>
                  {pav}
                </div>
              ))}
            </div>
            ...
            pavimentosAtividades.map((item, idx) => (
              <div key={idx} className="rdo-pavimento-linha">
                ...
                <div className="rdo-pavimento-descricao">
                  <textarea value={item.descricao} onChange={(e) => updatePavimentoDescricao(item.pavimento, e.target.value)} placeholder={`Descreva os serviços realizados no ${item.pavimento}...`} />
                </div>
                <button type="button" className="rdo-pavimento-remover" onClick={() => removerPavimentoDoRdo(item.pavimento)} aria-label={`Remover ${item.pavimento}`}>
                  <FaTrash />
                </button>
              </div>
            ))
        ...
        <div className="rdo-section rdo-efetivos-container">
          <div className="rdo-efetivo-col">
            <div className="rdo-section-header">
              ...
              <div className="rdo-add-button-group">
                <img src={dataTransferImage} alt="Copiar do último RDO" className="data-transfer-icon" onClick={() => copiarDoUltimoRdo("efetivo_proprio")} width="24" height="24" />
                <button type="button" onClick={() => addRow("efetivo_proprio")}>+</button>
              </div>
            </div>
            <table>
              <thead><tr><th>Função</th><th>Total</th><th>Presentes</th><th></th></tr></thead>
              <tbody>
                {data.efetivo_proprio.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td><button type="button" className="rdo-pavimento-remover" onClick={() => removeRow("efetivo_proprio", idx)}><FaTrash /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="rdo-efetivo-col">
            <div className="rdo-section-header">
              ...
              <div className="rdo-add-button-group">
                <img src={dataTransferImage} alt="Copiar do último RDO" className="data-transfer-icon" onClick={() => copiarDoUltimoRdo("efetivo_terceirizado")} width="24" height="24" />
                <button type="button" onClick={() => addRow("efetivo_terceirizado")}>+</button>
              </div>
            </div>
            <table>
              <thead><tr><th>Empresa</th><th>Total</th><th>Presentes</th><th></th></tr></thead>
              <tbody>
                {data.efetivo_terceirizado.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td><button type="button" className="rdo-pavimento-remover" onClick={() => removeRow("efetivo_terceirizado", idx)}><FaTrash /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rdo-section">
          <div className="rdo-section-header">
            <h3><FontAwesomeIcon icon={faTruck} /> Recebimento de Materiais</h3>
            <div className="rdo-add-button-group">
              <img src={dataTransferImage} alt="Copiar do último RDO" className="data-transfer-icon" onClick={() => copiarDoUltimoRdo("recebimento_materiais")} width="24" height="24" />
              <button type="button" onClick={() => addRow("recebimento_materiais")}>+</button>
            </div>
          </div>
          <table>
            <thead>
              <tr><th>Código</th><th>Descrição</th><th>Unidade</th><th>Quantidade</th><th>Empresa</th><th></th></tr>
            </thead>
            <tbody>
                ...
                <tr key={idx}>
                  <td>
                    <div className="codigo-com-lupa">
                      <input type="text" value={item.codigo || ""} onChange={(e) => updateArrayField("recebimento_materiais", idx, "codigo", e.target.value)} onBlur={() => buscarMaterialPorCodigo(idx, item.codigo)} onKeyPress={(e) => e.key === "Enter" && buscarMaterialPorCodigo(idx, item.codigo)} placeholder="Cód." />
                      <button type="button" className="lupa-busca-btn" onClick={() => abrirBuscaInsumo(idx, "recebimento_materiais")} title="Buscar material"><FaMagnifyingGlass /></button>
                    </div>
                  </td>
                  <td><input type="text" value={item.descricao || ""} readOnly className="rdo-input-readonly" placeholder="Descrição" /></td>
                  <td><input type="text" value={item.unidade || ""} readOnly className="rdo-input-readonly" placeholder="Un." /></td>
                  <td><input type="number" value={item.quantidade || ""} onChange={(e) => updateArrayField("recebimento_materiais", idx, "quantidade", e.target.value)} placeholder="0" /></td>
                  <td><input type="text" value={item.empresa || ""} onChange={(e) => updateArrayField("recebimento_materiais", idx, "empresa", e.target.value)} placeholder="Empresa" /></td>
                  <td><button type="button" className="rdo-pavimento-remover" onClick={() => removeRow("recebimento_materiais", idx)}><FaTrash /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="rdo-section">
          <div className="rdo-section-header">
            ...
            <div className="rdo-add-button-group">
              <img src={dataTransferImage} alt="Copiar do último RDO" className="data-transfer-icon" onClick={() => copiarDoUltimoRdo("equipamentos")} width="24" height="24" />
              <button type="button" onClick={() => addRow("equipamentos")}>+</button>
            </div>
          </div>
          <table>
            <thead><tr><th>Código</th><th>Descrição</th><th>Total</th><th>Em Uso</th><th></th></tr></thead>
            <tbody>
              {data.equipamentos.map((item, idx) => (
                <tr key={idx}>
                  <td>
                    <div className="codigo-com-lupa">
                      <input type="text" value={item.codigo || ""} onChange={(e) => updateArrayField("equipamentos", idx, "codigo", e.target.value)} onBlur={() => buscarItemPorCodigo(idx, item.codigo)} onKeyPress={(e) => e.key === "Enter" && buscarItemPorCodigo(idx, item.codigo)} placeholder="Cód." />
                      <button type="button" className="lupa-busca-btn" onClick={() => abrirBuscaInsumo(idx, "equipamentos")} title="Buscar insumo"><FaMagnifyingGlass /></button>
                    </div>
                  </td>
                  <td><input type="text" value={item.descricao || ""} readOnly className="rdo-input-readonly" placeholder="Descrição" /></td>
                  <td><input type="number" value={item.total || ""} onChange={(e) => updateArrayField("equipamentos", idx, "total", e.target.value)} placeholder="0" /></td>
                  <td><input type="number" value={item.em_uso || ""} onChange={(e) => updateArrayField("equipamentos", idx, "em_uso", e.target.value)} placeholder="0" /></td>
                  <td><button type="button" className="rdo-pavimento-remover" onClick={() => removeRow("equipamentos", idx)}><FaTrash /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setData(prev => ({
                    ...
                    intercorrencias: checked ? prev.intercorrencias : ""
                  }));
                }}
              />
            </label>
          ...
          {data.fotos.length > 0 && (
            <div className="rdo-fotos-preview">
              {data.fotos.map((foto, idx) => (
                <div key={idx} className="rdo-foto-item" onClick={() => { setFotoEmExibicao(idx); document.body.classList.add("no-scroll"); }}>
                  <img src={typeof foto === "string" ? foto : URL.createObjectURL(foto)} alt={`Foto ${idx + 1}`} className="rdo-foto-thumb" />
                  <button type="button" className="rdo-foto-remove" onClick={(e) => { e.stopPropagation(); setData(prev => ({ ...prev, fotos: prev.fotos.filter((_, i) => i !== idx) })); }} aria-label="Remover foto">
                    <FaTrash size={10} />
                  </button>
                </div>
              ))}
          ...
          {data.fotos.length < 4 && (
            <div className="rdo-fotos-actions">
              <label className="rdo-btn rdo-btn-primary">
                Escolher da galeria
                <input type="file" multiple accept="image/*" className="rdo-file-input" onChange={(e) => {
                  const files = Array.from(e.target.files || []);
                  const espaco = 4 - data.fotos.length;
                  if (files.length > espaco) { alert(`Você só pode adicionar até ${espaco} foto(s).`); return; }
                  setData(prev => ({ ...prev, fotos: [...prev.fotos, ...files] }));
                }} />
              </label>
              <button type="button" className="rdo-btn rdo-btn-success" onClick={() => { if (data.fotos.length >= 4) { alert("Limite de 4 fotos atingido."); return; } setCameraAberta(true); }}>
                Tirar foto
              </button>
            </div>
          )}
        ...
        <div className="rdo-actions">
          <div className="rdo-export-buttons">
            <button type="button" className="rdo-export-btn rdo-print-btn" onClick={() => window.print()} title="Imprimir RDO">
              <FontAwesomeIcon icon={faPrint} /><span>Impressão</span>
            </button>
            <button type="button" className="rdo-export-btn rdo-pdf-btn" onClick={() => RdoPdf.exportar(projetoNome, dataOriginal)} title="Exportar para PDF">
              <FontAwesomeIcon icon={faFilePdf} /><span>PDF</span>
            </button>
          </div>
          <div className="rdo-main-actions">
            <button className="btn-cancel" onClick={handleCancel} disabled={loading}>Cancelar</button>
            <button className="btn-save" onClick={saveRdo} disabled={loading}>{loading ? "Salvando..." : "Salvar RDO"}</button>
          </div>
        </div>
      </div>
      ...
      {/* Lightbox */}
      {fotoEmExibicao !== null && (
        <div className="rdo-lightbox" onClick={() => { setFotoEmExibicao(null); document.body.classList.remove("no-scroll"); }}>
          <div className="rdo-lightbox-content" onClick={(e) => e.stopPropagation()}>
            <img src={typeof data.fotos[fotoEmExibicao] === "string" ? data.fotos[fotoEmExibicao] : URL.createObjectURL(data.fotos[fotoEmExibicao])} alt={`Foto ${fotoEmExibicao + 1}`} className="rdo-lightbox-img" />
            {data.fotos.length > 1 && (
              <>
                <button className="rdo-lightbox-nav rdo-lightbox-prev" onClick={(e) => { e.stopPropagation(); setFotoEmExibicao(prev => prev === 0 ? data.fotos.length - 1 : prev - 1); }}>❮</button>
                <button className="rdo-lightbox-nav rdo-lightbox-next" onClick={(e) => { e.stopPropagation(); setFotoEmExibicao(prev => prev === data.fotos.length - 1 ? 0 : prev + 1); }}>❯</button>
              </>
            )}
            <div className="rdo-lightbox-counter">{fotoEmExibicao + 1} / {data.fotos.length}</div>
          </div>
        </div>
      )}

      <BuscaInsumo isOpen={buscaInsumoAberta} onClose={() => { setBuscaInsumoAberta(false); setLinhaBuscaAtiva(null); setTipoInsumoAtivo(null); }} onSelect={handleSelecionarInsumo} />
      <RdoCamera isOpen={cameraAberta} onClose={() => setCameraAberta(false)} onCapture={(fotoBlob) => { setData(prev => ({ ...prev, fotos: [...prev.fotos, fotoBlob] })); }} />
    </div>
  );
};
